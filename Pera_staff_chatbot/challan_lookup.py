...
def _parse_single_date(text: str, default_year: Optional[int] = None,
    ...
    # ── Yearless dates — use default_year or fall back to current year ──
    # e.g. "10 March" or "March 10" in a date-range like "10 march to 14 march"
    _year = default_year or date.today().year
...
def _lookup_location(loc_type: str, loc_name: str) -> Optional[Dict]:
    ...
    # Build verification query for the user
    table_map = {
        "tehsil": "challan_tehsil_drill",
        "district": "challan_by_district",
        "division": "challan_by_division",
    }
    tbl = table_map.get(loc_type, "challan_by_division")
    name_col = f"{loc_type}_name"
    verify_query = (
        f"SELECT * FROM {tbl} WHERE TRIM({name_col}) = '{loc_name}' ORDER BY status;"
    )
...
def _lookup_location_type(req_key: str, loc_type: str, loc_name: str) -> Optional[Dict]:
        ...
        if not records:
            ...
            if drill_rows:
                ...
                return _build_result(
                    ...
                    records, "\n".join(lines), "\n".join(ctx_lines),
                    data_table="challan_tehsil_drill",
                    data_query=f"SELECT * FROM challan_tehsil_drill\nWHERE TRIM(tehsil_name) = '{loc_name}'\nORDER BY status;",
                )
    ...
    # Build verification query for location+type
    loc_type_query = (
        f"SELECT status, tehsil_name, total_requisitions, {count_col}\n"
        f"FROM challan_tehsil_breakdown\n"
        f"WHERE TRIM(tehsil_name) IN (\n"
        f"  SELECT TRIM(dt.tehsil_name) FROM dim_tehsil dt\n"
        f"  JOIN dim_district dd ON dd.district_id = dt.district_id\n"
        f"  {'JOIN dim_division dv ON dv.division_id = dd.division_id' if loc_type == 'division' else ''}\n"
        f"  WHERE {'dv.division_name' if loc_type == 'division' else 'dd.district_name'} = '{loc_name}'\n"
        f")\nORDER BY tehsil_name, status;"
    ) if loc_type in ("district", "division") else (
        f"SELECT * FROM challan_tehsil_breakdown\n"
        f"WHERE TRIM(tehsil_name) = '{loc_name}'\nORDER BY status;"
    )

    return _build_result(
        "challan_location", f"PERA Challan — {loc_name} ({display_type})",
        records, "\n".join(lines), "\n".join(ctx_lines),
        data_table="challan_tehsil_breakdown",
        data_query=loc_type_query,
    )
...
def _format_daterange_summary(rows, date_label: str, title: str,
    ...
    # Build verification query — extract dates from the label
    dr_parts = date_label.split(" — ")
    dr_query = (
        f"-- Challan date range query: {date_label}\n"
        f"WITH latest_challan AS (\n"
        f"  SELECT DISTINCT ON (challan_id) *\n"
        f"  FROM challan_data\n"
        f"  WHERE action_date >= '{dr_parts[0]}'\n"
        f"    AND action_date <= '{dr_parts[1] if len(dr_parts) > 1 else dr_parts[0]}'\n"
        f"  ORDER BY challan_id, snapshot_date DESC\n"
        f")\n"
        f"SELECT status, COUNT(*) AS total_challans,\n"
        f"       SUM(fine_amount) AS total_fine\n"
        f"FROM latest_challan\nGROUP BY status ORDER BY status;"
    )

    return _build_result(
        "challan_daterange", title,
        rows, "\n".join(lines), "\n".join(ctx_lines),
        data_table="challan_data",
        data_query=dr_query,
    )
...
def _lookup_officer_at_location(loc_type: str, loc_name: str,
    ...
    # Build verification query for officers at location
    loc_col = {"tehsil": "tehsil_name", "district": "district_name", "division": "division_name"}.get(loc_type, "tehsil_name")
    oal_query = (
        f"SELECT officer_name, COUNT(*) AS total_challans,\n"
        f"       SUM(fine_amount) AS total_fine\n"
        f"FROM challan_data\n"
        f"WHERE {loc_col} ILIKE '%{loc_name}%'\n"
    )
    if req_key and req_key in _REQ_KEY_TO_DB_NAME:
        oal_query += f"  AND requisition_type_name = '{_REQ_KEY_TO_DB_NAME[req_key]}'\n"
    oal_query += "GROUP BY officer_name\nORDER BY total_challans DESC;"

    return _build_result(
        "challan_officer_at_location", title,
        records, "\n".join(lines), "\n".join(ctx_lines),
        data_table="challan_data",
        data_query=oal_query,
    )
...
def _lookup_comparison(status: str = "paid", level: str = "tehsil",
    ...
    # Build verification query matching the actual lookup
    comp_table_map = {"division": "challan_by_division", "district": "challan_by_district", "tehsil": "challan_tehsil_drill"}
    comp_tbl = comp_table_map.get(level, "challan_tehsil_drill")
    comp_name_col = {"division": "division_name", "district": "district_name", "tehsil": "tehsil_name"}.get(level, "tehsil_name")
    if req_key and req_key in _REQ_KEY_TO_DB_NAME:
        comp_query = (
            f"SELECT {comp_name_col} AS name, COUNT(*) AS total_challans,\n"
            f"       SUM(fine_amount) AS total_amount\n"
            f"FROM challan_data\n"
            f"WHERE requisition_type_name = '{_REQ_KEY_TO_DB_NAME[req_key]}'\n"
            f"GROUP BY {comp_name_col}\nORDER BY total_challans DESC;"
        )
        comp_tbl = "challan_data"
    else:
        comp_query = (
            f"SELECT * FROM {comp_tbl}\n"
            f"WHERE LOWER(status) = '{status.lower()}'\n"
            f"ORDER BY total_challans DESC;"
        )
...
def _lookup_officer(officer_name: str, question: str = "") -> Optional[Dict]:
    ...
    # Build verification query specific to this officer
    officer_verify = (
        f"SELECT status, COUNT(*) AS total_challans,\n"
        f"       SUM(fine_amount) AS total_fine,\n"
        f"       SUM(paid_amount) AS total_paid\n"
        f"FROM challan_data\n"
        f"WHERE officer_name ILIKE '%{officer_name}%'\n"
...
def _lookup_totals() -> Optional[Dict]:
    ...
    return _build_result(
        "challan_totals", "PERA Challan Totals", records, answer, context,
        data_table="challan_totals",
        data_query="SELECT * FROM challan_totals WHERE id = 1;",
    )
...
def _lookup_by_division() -> Optional[Dict]:
    ...
    return _build_result(
        ...
        records, "\n".join(lines), "\n".join(ctx_lines),
        data_table="challan_by_division",
        data_query="SELECT division_name, status, total_challans, total_amount\nFROM challan_by_division\nORDER BY division_name, status;",
    )
...
def _lookup_by_district() -> Optional[Dict]:
    ...
    return _build_result(
        ...
        records, "\n".join(lines), "\n".join(ctx_lines),
        data_table="challan_by_district",
        data_query="SELECT cbd.district_name, cbd.status, cbd.total_challans, cbd.total_amount,\n       dd.division_name\nFROM challan_by_district cbd\nLEFT JOIN dim_division dd ON dd.division_id = cbd.division_id\nORDER BY dd.division_name, cbd.district_name, cbd.status;",
    )
...
def _lookup_by_tehsil() -> Optional[Dict]:
    ...
    return _build_result(
        ...
        records, "\n".join(lines), "\n".join(ctx_lines),
        data_table="challan_by_tehsil",
        data_query="SELECT tehsil_name, status, count\nFROM challan_by_tehsil\nORDER BY tehsil_name, status;",
    )
...
def _lookup_requisition_type() -> Optional[Dict]:
    ...
    return _build_result(
        ...
        records, "\n".join(lines), "\n".join(ctx_lines),
        data_table="challan_requisition_type",
        data_query="SELECT requisition_type_name, status, total_challans, total_amount\nFROM challan_requisition_type\nORDER BY requisition_type_name, status;",
    )
...
def _lookup_tehsil_breakdown() -> Optional[Dict]:
    ...
    return _build_result(
        ...
        records, "\n".join(lines), "\n".join(ctx_lines),
        data_table="challan_tehsil_breakdown",
        data_query="SELECT tehsil_name, status, total_requisitions,\n       hoarding_count, price_control_count,\n       encroachment_count, land_retrieval_count, public_nuisance_count\nFROM challan_tehsil_breakdown\nWHERE total_requisitions > 0\nORDER BY total_requisitions DESC;",
    )
